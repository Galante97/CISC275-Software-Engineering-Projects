...
public class PopModel {
	...
	public void checkMatch() { // not complete yet
		System.out.println("Checking if Bubbles matched..");
				...
				for (int i = 0; i < grid.length; i++) {
					for (int j = 0; j < grid[0].length; j++) {
						...
						if (grid[i][j].xCoord == bub.xCoord && grid[i][j].yCoord == bub.yCoord) {
							grid[i][j] = null;
							System.out.println("A Bubble Matched!");
							// matchedList.remove();
						}
					}
				}
	...
	public void shoot(int degr, double xCo, double yCo) {
		System.out.println("Fireing at " + degr + " degrees");
